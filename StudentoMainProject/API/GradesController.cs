    ...
    public class GradesController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<GradeObject>>> TeacherGetGrades(int? studentId, int? subjectInstanceId, int gradeValueFormat = 0)
        {
            ...
            if (subjectInstanceId != null)
            {
                ...
                foreach (var g in grades)
                {
                    GradeObject newGrade = new() { Added = g.Added, Id = g.Id, Name = g.Name, StudentId = g.StudentId, SubjectInstanceId = g.SubjectInstanceId, GradeGroupId = g.GradeGroupId, GradeGroupName = g.GradeGroup?.Name, GradeGroupWeight = g.GradeGroup?.Weight, GradeGroupAdded = (DateTime)(g.GradeGroup?.Added) };
                    newGrade.Value = gradeValueFormat switch
                    {
                        //Internal
                        0 => g.GetInternalGradeValue().ToString(),
